...
namespace SFBMS.WebAPI.Infrastructure.IdentityServer4
{
    /// <summary>
    ...
    public class ResourceOwnerPasswordValidator : IResourceOwnerPasswordValidator
    {
        ...
        public ResourceOwnerPasswordValidator(IAdminRepository adminRepository)
        {
        ...
        {
            //判断授权范围是否包含三端
            if (Enum.TryParse<AppTypes>(context.Request.Scopes.FirstOrDefault(), true, out var appType))
            {
                //根据Scopes判断登入接口
                User user = null;
                switch (appType)
                {
                    //case AppTypes.Doctor:
                    //    user = await new ClientUserHandler(_UserRepository).GetUserAsync(context.UserName, context.Password);
                    //    break;
                    case AppTypes.Background:
